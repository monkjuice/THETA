...
int runPatternTest()
{
    try
    {
        ...
        session.undo();
        require(sequence.getNumNotes() == 2, "Undo clear");
        ...
        require(session.hasNote(4, 55), "Tempo change preserves note beats");
        session.undo();
        ...
        require(session.pattern().getSequence().getNumNotes() == 2, "Notes survive project reopen");
