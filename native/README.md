...
A C++20/Tracktion/JUCE application with a playable one-bar pattern editor, a two-track arrangement view with audio waveforms and clip editing, Tracktion's 4OSC synth, our internal Utility gain device, transport, audio hardware settings, and native project save/open.
...
- Draw notes in the 16-step grid; drag an existing note or right-drag to erase. Each stroke is one undo action. Click the grid to focus it; its cyan border marks where note shortcuts apply. Ctrl+A (Command+A on macOS) selects all notes; Ctrl/Command+C and V paste the full selected shape into the earliest empty grid cells, extending the MIDI clip when needed. Ctrl/Command+E divides a same-lane selection into retriggers; keep the modifier held and use the arrows or wheel to choose 2-32 divisions without changing the grid. Hold V and use Up/Down or the wheel to adjust selected-note velocity; the footer shows the common percentage or `MIXED`. Ctrl/Command+Z, Ctrl/Command+Shift+Z, and Ctrl/Command+Y undo and redo while the grid is focused. The initial octave is MIDI 48–59; pitch-name labels use C4 for middle C.
